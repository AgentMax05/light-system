...
# Raspberry Pi controls the LED strip directly
    ...
    strip.begin()
# Raspberry Pi with SK9822/APA102 LEDs via SPI
elif config.DEVICE == 'pi-sk9822':
    from led_sk9822 import SK9822Controller
    strip = SK9822Controller(
        num_leds=config.N_PIXELS,
        spi_bus=config.SPI_BUS,
        spi_device=config.SPI_DEVICE,
        max_speed_hz=config.SPI_SPEED_HZ,
        brightness=config.SK9822_BRIGHTNESS,
...
def _update_pi_sk9822():
    """Writes new LED values to SK9822/APA102 LED strip via SPI
    
    Uses the SK9822Controller class to update the LED strip.
    """
    global pixels
    # The SK9822Controller handles clipping, gamma correction, and SPI communication
    strip.update(pixels, gamma_table=_gamma if config.SOFTWARE_GAMMA_CORRECTION else None)
